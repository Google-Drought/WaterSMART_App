...
def get_auth():
    ...
    service_account_keys = st.secrets["GEE_CREDS"]
...
# with tab_map["GDE Explorer"]:
    ...
    *NOTE: This tool provides estimates of how a GDE would respond IF it existed at the selected location with the chosen characteristics. 
    ...
    layer_assets = {
        "Administrative groundwater boundaries": "projects/dri-apps/assets/WaterSMART/NVAdminGWBoundaries",
    ...
    if st.session_state.get_data_clicked and coords_ee is not None:
        ...
        try:
            ...
            if admin_gw.filterBounds(coords_ee).size().getInfo() == 1:
            ...
            elif admin_gw.filterBounds(coords_ee).size().getInfo() == 0:
                basin_id = "N/A"
                basin_name = "N/A"
            else:
                raise ValueError("Multiple basins found for the selected location.")
            ...
            if st.session_state.get_data_clicked:
                ...
                st.markdown("### Groundwater Evapotranspiration Analysis")
